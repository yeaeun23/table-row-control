/*  전자결재 양식 - 행 추가/삭제 Plug-In
                  Ver 1.1.0 (17.01.06) - 다중 행 추가 기능, rowspanClass 옵션 추가  */
...
$.fn.PlusMinusRow = function(options){
	...
	function plusRow(){
		...
		var copyTrCnt = $tr.length;

		// ① rowspan 처리
		var $rowspanTd = $("."+settings.rowspanClass);

		if($rowspanTd){
			$.each($rowspanTd, function(k, v){
				$(v).attr("rowspan", parseInt($(v).attr("rowspan")) + copyTrCnt);
			});
		}
